...
import { markLeadKeepMutation } from "@/lib/leads/mutations";
...
export async function markLeadKeep(
  ...
  const result = await markLeadKeepMutation(parsed.data.leadId, user.id);
